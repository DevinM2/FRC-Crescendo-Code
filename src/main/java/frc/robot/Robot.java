...
public class Robot extends TimedRobot {
  ...
  private static final String kSpit = "drive & spit";
  private static final String kTurn = "put note in amp";
  ...
   */
  CANSparkBase leftRear = new CANSparkMax(19, MotorType.kBrushed);
  CANSparkBase leftFront = new CANSparkMax(11, MotorType.kBrushed);
  CANSparkBase rightRear = new CANSparkMax(20, MotorType.kBrushed);
  ...
   */
  //CANSparkBase m_launchWheel = new CANSparkMax(8, MotorType.kBrushed);
  //CANSparkBase m_feedWheel = new CANSparkMax(10, MotorType.kBrushed);

  /**
   * Roller Claw motor controller instance.
  */
  CANSparkBase m_rollerClaw = new CANSparkMax(9, MotorType.kBrushed);
   ...
   */
  CANSparkBase m_climber = new CANSparkMax(2, MotorType.kBrushed);
  ...
   */
  Joystick m_driverControllerLeft = new Joystick(1);
  Joystick m_driverControllerRight = new Joystick(2);

  Joystick m_manipController = new Joystick(3);
 ...
 /**
   ...
   */
  static final double CLAW_OUTPUT_POWER = .4;
  /**
  ...
  public void robotInit() {
    ...
    m_chooser.addOption("drive forward", kDrive);
    m_chooser.addOption("drive and spit out note", kSpit);
    m_chooser.addOption("put note in amp", kTurn);
  ...
  double AUTO_DRIVE_FORWARD_FIRST;
  double AUTO_DRIVE_FORWARD_SECOND;
  double TIME_FOR_90_CCW;
  double SHOOT_TIME;

  double AUTO_DRIVE_SPEED;
  double AUTO_LAUNCHER_SPEED;
  double AUTO_CLAW_OUT_SPEED;
  ...
  public void autonomousInit() {
    ...
    AUTO_DRIVE_TIME_S = 3;
    //AUTO_DRIVE_TURN1_S =  ;
    AUTO_DRIVE_FORWARD_FIRST = 0; // Add time after trials
    AUTO_DRIVE_FORWARD_SECOND = 0; // Add time after trials
    TIME_FOR_90_CCW = 1.0; // Add time after trials
    SHOOT_TIME = 0; // Add time after trials
    
    // Speeds
    AUTO_DRIVE_SPEED = -0.5;
    AUTO_LAUNCHER_SPEED = 1;
    AUTO_CLAW_OUT_SPEED = CLAW_OUTPUT_POWER;
    ...
    } else if(m_autoSelected == kTurn) {

      AUTO_LAUNCHER_SPEED = 0;

    } else if(m_autoSelected == kSpit) {
  ...
  public void autonomousPeriodic() {
    ...
    else if(timeElapsed < AUTO_DRIVE_DELAY_S + AUTO_DRIVE_TIME_S)
    {
      //m_launchWheel.set(0);
      //m_feedWheel.set(0);
      m_drivetrain.tankDrive(AUTO_DRIVE_SPEED, AUTO_DRIVE_SPEED);
    }
    else
    {
      m_drivetrain.tankDrive(0, 0);
      m_rollerClaw.set(AUTO_CLAW_OUT_SPEED);
    }
    /* For an explanation on differintial drive, squaredInputs, arcade drive and tank drive see the bottom of this file */

    // Go Forward and Turn to Put the Note in the Amp
    /*
     * if(timeElapsed < AUTO_DRIVE_FORWARD_FIRST) {
     * 
     * m_drivetrain.tankDrive(AUTO_DRIVE_SPEED, AUTO_DRIVE_SPEED);
     * 
     * } else if(timeElapsed < AUTO_DRIVE_FORWARD_FIRST + TIME_FOR_90_CCW) {
     * 
     * m_drivetrain.tankDrive(0, AUTO_DRIVE_SPEED);
     * 
     * } else if(timeElapsed < AUTO_DRIVE_FORWARD_FIRST + TIME_FOR_90_CCW + AUTO_DRIVE_FORWARD_SECOND) {
     * 
     * m_drivetrain.tankDrive(AUTO_DRIVE_SPEED, AUTO_DRIVE_SPEED);
     * 
     * } else if(timeElapsed < AUTO_DRIVE_FORWARD_FIRST + TIME_FOR_90_CCW + AUTO_DRIVE_FORWARD_SECOND + SHOOT_TIME) {
     * 
     * m_drivetrain.tankDrive(0, 0);
     * m_rollerclaw.set(AUTO_CLAW_OUT_SPEED);
     * 
  ...
  public void teleopPeriodic() {
     ...
     */ 
    if(m_manipController.getRawButton(4)) //LogiTech Yellow/Intake Button
    {
      m_rollerClaw.set(-(CLAW_OUTPUT_POWER)); 
    }
    else if(m_manipController.getRawButton(3)) //LogiTech Red/Outtake Button
    {
      m_rollerClaw.set(CLAW_OUTPUT_POWER + 0.3);
    }
    else
    {
      m_rollerClaw.set(0);
     ...
     */
    if(m_manipController.getPOV() == 180) //hold up to climb
    {
      m_climber.set(1);            //to switch this (down to climb), flip the 180 and the 0 for getPOV()
    }
    ...
    if(m_driverControllerLeft.getRawButton(1)){
      slow = 0.6;
    } else{
      slow = 1;
    }
    //     if(m_driverControllerRight.getRawButton(1)){
    //   fast = 0.2;
    // } else{
    //   fast = 1;
    // }
    ...
    m_drivetrain.tankDrive(-0.98*slow*Math.atan(m_driverControllerLeft.getRawAxis(1)), -0.98*slow*Math.atan(m_driverControllerRight.getRawAxis(1)));
    //m_drivetrain.tankDrive(-0.9*fast*Math.atan(m_driverControllerLeft.getRawAxis(1)), -0.9*fast*Math.atan(m_driverControllerRight.getRawAxis(1)));
  }
}
